...
public class ParquetConverterService {
    ...
    public Resource convertJson(MultipartFile inputFile, Format inputFormat) throws Exception {
        List<Map<String, Object>> data = readData(inputFile, inputFormat);
        return writeUtil.writeToJson(data);
    }
    public Resource convertCsv(MultipartFile inputFile, Format inputFormat) throws Exception {
        List<Map<String, Object>> data = readData(inputFile, inputFormat);
        return writeUtil.writeToCsv(data);
    }
    public Resource convertXml(MultipartFile inputFile, Format inputFormat) throws Exception {
        List<Map<String, Object>> data = readData(inputFile, inputFormat);
        return writeUtil.writeToXml(data);
    }
    public Resource convertParquet(MultipartFile inputFile, Format inputFormat) throws Exception{
        List<Map<String, Object>> data = readData(inputFile, inputFormat);
        return writeUtil.writeToParquet(data);
    }

    private List<Map<String, Object>> readData(MultipartFile file, Format format) throws Exception {
        return switch (format) {
            ...
            case KML -> readUtil.readKml(file);
        };
    }
}